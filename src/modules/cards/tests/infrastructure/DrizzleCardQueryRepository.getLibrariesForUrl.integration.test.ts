...
describe('DrizzleCardQueryRepository - getLibrariesForUrl', () => {
  ...
  describe('complex scenarios', () => {
    it('should handle multiple users with multiple cards for same URL', async () => {
      const testUrl = 'https://example.com/complex-scenario';
      const url = URL.create(testUrl).unwrap();

      // Curator1: 2 cards with same URL
      const card1a = new CardBuilder()
        .withCuratorId(curator1.value)
        .withType(CardTypeEnum.URL)
        .withUrl(url)
        .buildOrThrow();

      const card1b = new CardBuilder()
        .withCuratorId(curator1.value)
        .withType(CardTypeEnum.URL)
        .withUrl(url)
        .buildOrThrow();

      // Curator2: 1 card with same URL
      const card2 = new CardBuilder()
        .withCuratorId(curator2.value)
        .withType(CardTypeEnum.URL)
        .withUrl(url)
      ...
      // Curator3: 1 card with different URL (should not appear)
      const differentUrl = URL.create('https://example.com/different').unwrap();
      const card3 = new CardBuilder()
        .withCuratorId(curator3.value)
        .withType(CardTypeEnum.URL)
        .withUrl(differentUrl)
        .buildOrThrow();

      // Add all cards to their respective libraries
      card1a.addToLibrary(curator1);
      card1b.addToLibrary(curator1);
      card2.addToLibrary(curator2);
      card3.addToLibrary(curator3);

      await cardRepository.save(card1a);
      await cardRepository.save(card1b);
      await cardRepository.save(card2);
      await cardRepository.save(card3);
      ...
      // Should return 3 entries: 2 from curator1, 1 from curator2
      expect(result.items).toHaveLength(3);
      expect(result.totalCount).toBe(3);

      // Check user distribution
      const userIds = result.items.map((lib) => lib.userId);
      const curator1Count = userIds.filter((id) => id === curator1.value).length;
      const curator2Count = userIds.filter((id) => id === curator2.value).length;
      const curator3Count = userIds.filter((id) => id === curator3.value).length;

      expect(curator1Count).toBe(2);
      expect(curator2Count).toBe(1);
      expect(curator3Count).toBe(0); // Different URL

      // Check card IDs
      const cardIds = result.items.map((lib) => lib.cardId);
      expect(cardIds).toContain(card1a.cardId.getStringValue());
      expect(cardIds).toContain(card1b.cardId.getStringValue());
      expect(cardIds).toContain(card2.cardId.getStringValue());
      expect(cardIds).not.toContain(card3.cardId.getStringValue());
    });

    it('should handle URL normalization edge cases', async () => {
      // Test with URLs that might be considered the same but are stored differently
      const baseUrl = 'https://example.com/article';
      const urlWithTrailingSlash = 'https://example.com/article/';
      const urlWithQuery = 'https://example.com/article?utm_source=test';

      const url1 = URL.create(baseUrl).unwrap();
      const url2 = URL.create(urlWithTrailingSlash).unwrap();
      const url3 = URL.create(urlWithQuery).unwrap();
      ...
      // Query for exact base URL
      const result = await queryRepository.getLibrariesForUrl(baseUrl, {
        page: 1,
        limit: 10,
        sortBy: CardSortField.UPDATED_AT,
        sortOrder: SortOrder.DESC,
  ...
  describe('error handling', () => {
    it('should handle database errors gracefully', async () => {
      ...
      await expect(
        queryRepository.getLibrariesForUrl(testUrl, {
          page: 1,
          limit: 10,
          sortBy: CardSortField.UPDATED_AT,
          sortOrder: SortOrder.DESC,
