...
import { AnnotationMapper } from "../../infrastructure/persistence/mappers/AnnotationMapper";
import { AnnotationNote } from "../../domain/value-objects/AnnotationNote";
...
export class FillAnnotationTemplateUseCase {
  ...
  async execute(input: FillAnnotationTemplateInputDTO): Promise<any> {
    ...
    //   input.templateRefUri
    // ); // Assuming template.id is TID
    ...
    //       default:
    //         throw new Error(
    //           `Unsupported annotation value type: ${rawValue?.$type}`
    //         );
    //     }
    //   } catch (error: any) {
    //     throw new Error(
    ...
    //     note: input.note ? AnnotationNote.create(input.note) : undefined,
    //   });
