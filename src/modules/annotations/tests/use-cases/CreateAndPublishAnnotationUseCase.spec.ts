...
import { CreateAndPublishAnnotationUseCase } from '../../application/use-cases/CreateAndPublishAnnotationUseCase';
...
describe('CreateAndPublishAnnotationUseCase', () => {
  ...
  it('should create and publish a valid annotation', async () => {
    ...
    if (result.isOk()) {
      ...
      const savedAnnotation = await annotationRepository.findById(
        annotationIdVO,
      );
