...
describe('CitoidMetadataService Integration Tests', () => {
  ...
  describe('fetchMetadata', () => {
    ...
    it('should handle invalid URLs gracefully', async () => {
      ...
      if (result.isErr()) {
        expect(result.getError().message).toContain('metadata');
      } else {
    ...
    it('should handle network errors gracefully', async () => {
      ...
      if (result.isErr()) {
        expect(result.getError().message).toContain('Failed to fetch metadata from Citoid');
      }
    }, 10000);
  });
    ...
    it('should parse publication dates correctly', async () => {
      ...
      if (result.isOk()) {
        ...
        if (metadata.publishedDate) {
          ...
          expect(metadata.publishedDate.getTime()).toBeLessThanOrEqual(now.getTime());
          expect(metadata.publishedDate.getTime()).toBeGreaterThan(oneYearAgo.getTime());
          
          console.log('Parsed publication date:', metadata.publishedDate.toISOString());
        }
      }
    }, 10000);
  });
