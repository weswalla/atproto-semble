...
import { AnnotationFieldInputDTO, AnnotationFieldOutputDTO } from '../dtos/AnnotationFieldDTO'
import {
  FieldDefinition,
  DyadFieldDef,
  TriadFieldDef,
...
export class CreateAnnotationFieldUseCase {
  ...
  async execute(input: AnnotationFieldInputDTO): Promise<AnnotationFieldOutputDTO> {
    ...
    switch (defInput?.$type) {
      ...
      case 'app.annos.annotationField#triadFieldDef':
        definition = new TriadFieldDef(defInput.vertexA, defInput.vertexB, defInput.vertexC)
        break
      ...
      default:
        ...
        throw new Error(`Invalid or unknown field definition type: ${defInput?.$type}`)
    }
