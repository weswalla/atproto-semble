...
export class FakeAnnotationsFromTemplatePublisher
  implements IAnnotationsFromTemplatePublisher
{
  ...
  async publish(
    ...
    const templateId =
      annotationsFromTemplate.template.templateId.getStringValue();
    ...
    for (const annotation of annotationsFromTemplate.annotations) {
      ...
      publishedRecordIds.set(annotationId.getStringValue(), publishedRecordId);
  ...
  async unpublish(
    ...
    for (const recordId of recordIds) {
      ...
      if (this.publishedAnnotations.has(compositeKey)) {
        ...
        console.log(
          `[FakeAnnotationsFromTemplatePublisher] Unpublished record ${recordId.uri}`
        );
      } else {
        console.warn(
