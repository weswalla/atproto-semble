...
describe("HighlightCardContent", () => {
  ...
  describe("HTML document annotation (browser extension simulation)", () => {
    ...
    function simulateTextSelection(startNode: Node, startOffset: number, endNode: Node, endOffset: number) {
    ...
    function findTextPosition(searchText: string, documentText: string): { start: number; end: number } | null {
    ...
    it("should create highlight from browser text selection", () => {
      ...
      const selectedText = "Web annotations are becoming increasingly important";
      
      // Create a range selection (simulating browser selection)
      const range = simulateTextSelection(textNode, 0, textNode, selectedText.length);
      ...
      const result = HighlightCardContent.create(selectedText, [
        textQuoteSelector,
        textPositionSelector,
        rangeSelector
      ], {
    ...
    it("should handle cross-element text selection", () => {
      ...
      const range = simulateTextSelection(startText, startOffset, endText, endOffset);
    ...
    it("should handle blockquote selection", () => {
      ...
      const range = simulateTextSelection(textNode, startOffset, textNode, endOffset);
      ...
      const result = HighlightCardContent.create(selectedPortion, [
        textQuoteSelector,
        rangeSelector
      ], {
        context: quoteText,
    ...
    it("should generate robust selectors for annotation recovery", () => {
      ...
      const prefix = fullText.substring(Math.max(0, startIndex - 20), startIndex);
      const suffix = fullText.substring(startIndex + targetText.length, startIndex + targetText.length + 20);
      ...
      const textQuoteSelector = content.getTextQuoteSelector()!;
      const searchPattern = textQuoteSelector.prefix + textQuoteSelector.exact + textQuoteSelector.suffix;
      ...
      const extractedText = documentText.substring(positionSelector.start, positionSelector.end);
    ...
    it("should handle edge cases in HTML documents", () => {
      ...
      // Select entire title
      const range = simulateTextSelection(title!.firstChild!, 0, title!.firstChild!, titleText.length);
