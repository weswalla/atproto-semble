...
import { AuthMiddleware, AuthenticatedRequest } from '../shared/infrastructure/http/middleware/AuthMiddleware';
import { MockTokenRepository } from './mocks/MockTokenRepository';
...
describe('Auth Integration Tests', () => {
  ...
  beforeAll(async () => {
    // Setup
    const mockTokenRepo = new MockTokenRepository();
    ...
    // Protected route
    app.get('/api/protected', authMiddleware.ensureAuthenticated(), (req: AuthenticatedRequest, res) => {
      res.json({ message: 'Protected route', did: req.did });
    });

    // Optional auth route
    app.get('/api/optional', authMiddleware.optionalAuth(), (req: AuthenticatedRequest, res) => {
      res.json({ message: 'Optional auth route', did: req.did || 'anonymous' });
    });
