import { ICollectionPublisher, CollectionPublishResult } from "../../application/ports/ICollectionPublisher";
...
export class FakeCollectionPublisher implements ICollectionPublisher {
  ...
  private publishedLinks: Map<string, { cardId: string; linkRecord: PublishedRecordId }[]> = new Map();
  ...
  async publish(collection: Collection): Promise<Result<CollectionPublishResult, UseCaseError>> {
    ...
    const publishedLinks: Array<{ cardId: string; linkRecord: PublishedRecordId }> = [];
  ...
  async unpublish(recordId: PublishedRecordId): Promise<Result<void, UseCaseError>> {
    ...
    for (const [collectionId, collection] of this.publishedCollections.entries()) {
      if (collection.publishedRecordId?.uri === recordId.uri) {
        ...
        console.log(`[FakeCollectionPublisher] Unpublished collection ${recordId.uri}`);
        return ok(undefined);
      }
    }
  ...
  getPublishedLinksForCollection(collectionId: string): Array<{ cardId: string; linkRecord: PublishedRecordId }> {
