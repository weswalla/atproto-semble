...
//         searchParams.set("error_description", params.error_description);

//       // Call the library's callback method
//       // This implicitly uses the sessionStore and stateStore provided during client setup
//       const { session, state: originalState } =
//         await this.oauthClient.callback(searchParams);

//       // Map the library's Session object to our ProcessedOAuthCallbackResult
//       const did = DID.create(session.did);

//       return {
//         did,
//         state: originalState || undefined, // Pass back the original state if needed
//       };
//     } catch (error: any) {
